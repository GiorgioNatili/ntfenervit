...
class EventSearchForm(HighlightedModelSearchForm):
    ...
    districtmanager = forms.CharField(max_length=100,required=False)
    ...
    def search(self):
        ...
        if self.cleaned_data['districtmanager']:
            sqs = sqs.filter(content = self.cleaned_data['districtmanager'])
